...
## Интернационализация и локализация в Java

### Интернационализация (i18n)

- Это процесс проектирования и разработки приложения таким образом, чтобы оно могло поддерживать различные языки и региональные настройки без необходимости перекомпиляции или перестройки.  
- В Java достигается за счёт отделения локализованных данных (тексты, форматы дат, чисел, валют) в отдельные ресурсы, которые загружаются динамически во время работы приложения.  
- Позволяет легко добавлять новые языки и регионы, просто добавляя соответствующие ресурсы.

### Локализация (l10n)

- Это процесс адаптации приложения под конкретный язык и регион.  
- Включает перевод текстов, настройку форматов даты, времени, чисел и валют согласно культурным особенностям региона.  
- Локализация использует результаты интернационализации, подставляя нужные ресурсы для выбранной локали.

---

### Хранение локализованных ресурсов

- В Java локализованные строки и данные обычно хранятся в **файлах свойств** (`.properties`), где каждый файл соответствует определённой локали.  
- Формат файла: пары ключ-значение (`key=value`), где ключ — идентификатор текста, а значение — перевод.  
- Пример:  
  - `messages_en.properties` — английская локализация  
  - `messages_ru.properties` — русская локализация  
- Для поддержки кириллицы в `.properties` файлах часто используется Unicode-экранирование (например, `\u041F\u0440\u0438\u0432\u0435\u0442`).

- Для загрузки и работы с такими ресурсами используется класс `ResourceBundle`, который автоматически выбирает нужный файл по текущей локали.  

```java
Locale locale = Locale.forLanguageTag("ru_RU");
...
- Представляет конкретную локаль — язык, страну и вариант.  
- Пример создания:  
  ```java
  Locale locale = new Locale("ru", "RU"); // русский язык, Россия
  ```
- Используется для выбора нужных ресурсов и форматирования данных.

---

### Итог

- **Интернационализация** — подготовка приложения к поддержке множества языков и регионов без изменений кода.  
- **Локализация** — адаптация приложения под конкретный язык и регион с помощью перевода и настройки форматов.  
- **Хранение локализованных ресурсов** — в отдельных файлах `.properties`, загружаемых динамически через `ResourceBundle`.  
- **Класс Locale** — описывает язык и регион для выбора ресурсов и форматирования.

---

Эти механизмы позволяют создавать гибкие и удобные многоязычные приложения, которые легко расширять и поддерживать.  

**Вопрос 43**

Форматирование локализованных числовых данных, текста, даты и времени в Java осуществляется с помощью специальных классов, учитывающих региональные настройки пользователя (локаль). Это позволяет отображать данные в привычном для конкретного языка и региона формате.

## Форматирование числовых данных

- Для чисел используется класс `NumberFormat` из пакета `java.text`.  
- Он автоматически форматирует числа с учётом локали (разделители тысяч, десятичные знаки, валютные символы).  
- Пример:
...
String formatted = numberFormat.format(number);  // "1 234 567,89"
```

- Для валюты используется `NumberFormat.getCurrencyInstance(locale)`.

## Форматирование текста

- Для интернационализации текста используют `ResourceBundle` для загрузки локализованных строк.  
- Форматирование строк с параметрами удобно делать с помощью `MessageFormat`:

```java
Locale locale = new Locale("ru", "RU");
ResourceBundle bundle = ResourceBundle.getBundle("messages", locale);
String pattern = bundle.getString("greeting");  // например, "Привет, {0}!"
String message = MessageFormat.format(pattern, "Иван");
```

## Форматирование даты и времени

- Для работы с датами и временем используют классы из пакета `java.time.format` и `java.time`.  
- Класс `DateTimeFormatter` поддерживает локализованные форматы.  
- Пример:
...
String formattedDate = date.format(formatter);  // "среда, 12 июня 2025 г."
```

- Аналогично можно форматировать время и дату-время.

---

Таким образом, в Java для локализованного форматирования чисел, текста, даты и времени используются классы `NumberFormat`, `MessageFormat`, `ResourceBundle` и `DateTimeFormatter` с указанием нужной локали, что обеспечивает корректное отображение данных согласно региональным стандартам.  

(Опираясь на интерес к интернационализации и локализации из памяти[1])



**Вопрос 44**

В Java графический интерфейс строится из компонентов, которые представлены в разных библиотеках и классах. Рассмотрим ключевые классы компонентов: **Component**, **JComponent** и **Node**.

---

## 1. Класс `Component` (AWT)

- Абстрактный базовый класс для всех визуальных элементов в библиотеке AWT (`java.awt.Component`).  
- Определяет основные свойства компонента: размер, положение, видимость, активность, цвет и т.д.  
- Обеспечивает обработку событий и методы для рисования (`paint(Graphics g)`, `repaint()`).  
- Компоненты могут быть вложены друг в друга через контейнеры (`Container` — наследник `Component`).  
- Используется в более старой библиотеке AWT, которая тесно связана с нативными элементами ОС.

---

## 2. Класс `JComponent` (Swing)

- Наследник `java.awt.Container`, который в свою очередь наследует `Component`.  
- Абстрактный базовый класс для всех компонентов Swing (`javax.swing.JComponent`).  
- Предоставляет расширенный функционал по сравнению с AWT-компонентами:  
  - Поддержка всплывающих подсказок (`setToolTipText()`).  
  - Встроенная двойная буферизация для сглаживания отрисовки.  
  - Поддержка MVC-подхода (Model-View-Controller).  
  - Расширенные возможности по кастомизации отрисовки через переопределение `paintComponent(Graphics g)`.  
- Все визуальные компоненты Swing (кнопки, панели, списки и т.д.) наследуются от `JComponent`.  
- Является "лёгким" компонентом — не зависит от нативных оконных ресурсов ОС, что обеспечивает кроссплатформенность и гибкость.

---

## 3. Класс `Node` (JavaFX)

- В JavaFX (современной библиотеке для создания GUI) базовый класс для всех визуальных элементов — `javafx.scene.Node`.  
- Представляет элемент сцены (scene graph), который может быть отображён и взаимодействовать с пользователем.  
- Все элементы интерфейса JavaFX (кнопки, панели, текст и др.) наследуются от `Node`.  
- Поддерживает сложную иерархию, анимации, эффекты и современный стиль программирования GUI.  
- Отличается от Swing/AWT по архитектуре и принципам работы.

---

## Итог

| Класс       | Библиотека         | Назначение и особенности                           |
|-------------|--------------------|---------------------------------------------------|
| **Component** | AWT (`java.awt`)     | Базовый класс для старых GUI-компонентов, с нативной отрисовкой и событиями. |
| **JComponent** | Swing (`javax.swing`) | Расширенный базовый класс для Swing-компонентов с поддержкой двойной буферизации и MVC. |
| **Node**      | JavaFX (`javafx.scene`) | Базовый элемент сцены в JavaFX с современными возможностями для UI. |

---

Таким образом, в Java GUI компоненты строятся на основе этих классов в зависимости от используемой библиотеки: AWT, Swing или JavaFX. `Component` — основа для AWT, `JComponent` — для Swing, а `Node` — для JavaFX.  

 Java. Пособие для проведения практических занятий. Занятие 9. http://rv-lab.ru/it/JAVA/lesson9.htm
